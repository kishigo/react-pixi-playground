...
import bunnys from "../img/doraemon.png";
...
class Bunnymark extends Component {
	...
	componentDidMount() {
		...
		// Add timer to add bunnies every 30 seconds
		setInterval(() => {
		...
		return (
			<Fragment>
				<ParticleContainer maxSize={maxSize} properties={particleContainerProperties}>
					{bunnys.map((bunny, i) => (
						<Sprite key={i} anchor={bunnyAnchor} texture={this.bunnyTextures[bunny.texture]} x={bunny.x}
						        y={bunny.y}/>
					))}
				</ParticleContainer>
